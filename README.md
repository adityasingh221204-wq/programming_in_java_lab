# [Flexi_even_UG_Programming Logic with Java Lab.pdf](https://github.com/user-attachments/files/25326329/Flexi_even_UG_Programming.Logic.with.Java.Lab.pdf)
# Programming Logic with Java Lab

This repository contains the laboratory assignments and projects completed as part of the **Programming Logic with Java Lab** course at **Symbiosis Institute of Technology, Pune**. The course focuses on applying **Object-Oriented Programming (OOP)** concepts to build modular, reusable, and scalable software solutions.

---

## 🚀 Learning Objectives
The primary goals of these lab sessions are:
* **Applying OOP concepts** for modular software design.
* **Designing real-life solutions** using inheritance, polymorphism, and interfaces.
* **Developing hands-on skills** in exception handling and generic coding.
* **Managing data** through File I/O and Database Connectivity (JDBC).
* **Building applications** including console-based and JavaFX applications.

---

## 📂 Lab Assignments & Progress Tracker

Use the table below to keep track of your progress throughout the semester. You can manually check off the tasks and enter the completion dates.

| Status | Lab No. | Assignment Description | Completion Date |
| :---: | :---: | :--- | :---: |
| ✅ | **1** | **Menu-driven Calculator:** Operators, control structures, and user input. | 13/01/2026 |
| ✅ | **2** | **Vehicle Management System:** Classes, getters/setters, and arrays. | 20/01/2026 |
| ✅ | **3** | **Book Management:** Constructors and exception handling with ArrayList. | 27/01/2026 |
| ✅ | **4** | **Vector Class:** 2D/3D vector math with exception handling. | 03/02/2026 |
|  | **5** | **Banking Application:** Inheritance hierarchy implementation. |  |
|  | **6** | **Employee Payroll System:** Multi-level/Multiple inheritance demo. |  |
|  | **7** | **Student Record Manager:** File Handling (CRUD) and exceptions. |  |
|  | **8** | **Design Patterns:** Application using Java Collections framework. |  |
|  | **9** | **Database Connectivity:** Backend integration using JDBC. |  |
|  | **10** | **JavaFX Application:** Building a modern GUI application. |  |

---

## 🛠 Pedagogy & Evaluation
The learning process integrates several modern practices:
* **Live Coding Sessions** and Laboratory Demonstrations.
* **Concept-Based Teaching** and Interactive Problem-Solving.
* **Integration of AI Tools** and Industry-Level Practices.

**Evaluation Criteria:**
* Lab Assignments & Quiz
* Mini Project
* Code Portfolio Submission

---

## 📚 Recommended Resources
### Textbooks
* *Java: The Complete Reference* by **Herbert Schildt** (Oracle Press).
* *Head First Java* by **Kathy Sierra and Bert Bates** (O'Reilly).

### Reference Books
* *Let Us Java* by **Yashavant Kanetkar** (BPB Online).
* *Core Java: An Integrated Approach* by **R. Nageswara Rao** (DreamTech Press).

---

**Course Designer:** Prof. Mayur Gaikwad, SIT Pune.
